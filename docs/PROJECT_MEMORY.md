...
## 当前系统阶段总判断（2026-06-11）

本节作为当前最高优先级记忆。如果本文件后文保留的历史过程记录与本节冲突，以本节为准。

### 当前系统定位判断

- 当前系统已经不是“单点扫码库存 Demo”，而是“国际贸易 ERP 全链路演示版”。
- 当前系统仍然是 Demo，不是正式商用 ERP。
- 当前系统的真实能力边界仍然是：`核心链路真实闭环 + 外围模块成熟展示 + 后续逐步业务化深化`。
- 当前可以明确认定：
  - `Demo 1.0` 核心真实闭环已完成。
  - `Demo 1.5` 成熟 ERP 模块包装已完成到 `阶段 19`。
  - 当前唯一尚未完成的既定大阶段是 `阶段 20：数据报表与业务大盘`。
...
- 核心真实闭环已具备：
  ...
  - 手机网页扫码入库
  - 手机网页扫码出库
  - 基于 `QrItem.status` 与 `StockMovement` 的真实库存统计
  - AI 基于真实受控数据回答库存与流程现状
- 成熟展示模块已具备：
  - 首页驾驶舱
  - 采购与集货
  - 国际物流
  ...
  - 多公司主体
  - 自动工单中心
- 当前工单与 AI 助手已经不再只是静态展示：
  - 工单会随关键业务节点逐步生成
  - AI 已能回答“当前走到哪一步、卡在哪、下一步做什么”
  - AI 回答已按“不懂业务的领导”口径做过防误解补强

### 当前与成熟 ERP 的关键差距判断

- 当前还缺 `阶段 20：数据报表与业务大盘`，因此经营层总览仍不完整。
- 当前外围模块虽然已成型，但多数仍是“可讲故事的业务展示”，不是“完整真实业务执行”。
- 当前主数据仍偏轻量，距离成熟 ERP 需要的 SKU、客商、人员、车辆、组织主数据体系还有差距。
- 当前合同、单据、采购、物流、清关、销售、财务虽然已串成链路，但很多规则仍未做到正式业务化深度。
- 当前二维码体系仍以“一箱一码”为主，距离成熟 ERP 常见的多级码、库位、盘点、逆向流程还有差距。
- 当前多公司仍以展示为主，还没有真正做到按公司 / 国家 / 岗位的数据隔离与权限控制。
- 当前系统底座仍以 Demo 运行优先，距离正式系统常见的并发、幂等、备份、对象存储、PostgreSQL 方案还有差距。

### 当前下一版本开发总原则

- 先完成既定 `阶段 20`，把 Demo 1.5 的最后一块经营展示补齐。
- `阶段 20` 完成后，再进入 `Demo 2.0：接近真实业务系统`。
- `Demo 2.0` 的目标不是推翻当前 Demo 重做，而是在现有真实闭环之上逐层业务化深化。
- 后续开发优先顺序固定为：
  - 主数据
  - 合同与单据业务化
  - 多级二维码与仓储执行
...
## Demo 2.0：接近真实业务系统路线图

### 总目标

- 在不破坏当前 Demo 可演示性的前提下，把系统逐步推进到“接近真实业务系统”的形态。
- 保持“核心链路真实、外围模块逐步真实化”的方法，不一次性追求企业级全量复杂度。

### 路线拆分

- `阶段 21：基础主数据强化`
  - 把 SKU、客商、人员、车辆等底座补成后续业务都能复用的主数据体系。
- `阶段 22：合同与单据业务化深化`
  - 把合同类型、执行控制、单据包完整性、版本差异留痕补齐。
- `阶段 23：多级二维码与仓储执行深化`
  - 把一箱一码扩展到多级码、库位、盘点、异常验收、双岗复核等更真实的仓储执行。
- `阶段 24：采购、物流、清关真实任务化`
  - 把采购、Shipment、清关资料包、异常处理从演示状态推进到真实任务状态。
- `阶段 25：销售、配送与售后闭环`
  - 把销售、配送、签收、售后、逆向库存与逆向财务串起来。
- `阶段 26：财务、发票与成本真实化`
  - 把应收应付、账期、发票、汇率、成本归集、利润追溯补齐到真实业务口径。
- `阶段 27：多公司权限与数据隔离`
  - 把当前“组织展示”升级为按公司、国家、岗位隔离的数据与菜单权限体系。
- `阶段 28：异常、归档与系统底座`
  - 把异常单、归档、审计留痕、并发保护、数据库与存储底座补齐。

### 当前执行结论

- 现在还不直接进入 `阶段 21`。
- 当前先完成 `阶段 20：数据报表与业务大盘`。
- `阶段 20` 之后，再严格按 `阶段 21 -> 阶段 28` 的顺序推进。
...
- 阶段 18：已完成，且已提交并推送
  - Commit: `d62384d`
  - Message: `feat: 完成阶段18多公司主体展示模块`
- 阶段 19：已完成，且已提交并推送
  - Commit: `51e1de7`
  - Message: `feat: 完善工单中心与AI流程现状问答`
- 阶段 20：未开始，为当前下一步
...
- 当时的“批次追踪 / 批次详情”页面曾增加：
  - 生成本批次二维码入口
  - 查看二维码追溯入口
- 当前最高优先级口径已按阶段 23 信息架构修正调整：
  - 二维码生成动作已归位到 `二维码追溯`
  - `批次追踪` 只保留查看、追踪和跳转，不再承载主要业务写入动作
...
- 本次已完成阶段 20 的真实页面落地，不再是占位页：
  - 页面位置：`/reports`
  - 文件：`apps/web/src/pages/ReportsPage.tsx`
  - 样式：`apps/web/src/styles.css`
- 本次数据报表页已经按“真实统计优先、演示口径补位”的原则实现：
  - `库存报表`、`在途报表`、`二维码追溯` 只取真实库存统计接口
  - `销售报表` 主值优先使用真实已出库数量，避免把销售计划误讲成已完成出库
  - `采购报表`、`回款报表`、`成本报表`、`利润报表` 在没有正式业务记录时，明确标注为 `Demo演示`
  - 一旦采购、财务、成本模块存在正式业务记录，报表页会优先显示业务记录而不是继续显示演示值
- 本次报表页复用了现有接口，没有新造一套重复后端：
  ...
  - `GET /api/finance/receivables`
  - `GET /api/costs/contracts`
- 本次报表页在“空白演示起点”下也能正常展示：
  - 真实库存、二维码、出入库数据保持 `0`
  - 经营类卡片使用默认演示场景补足 ERP 展示感
  - 页面上已经明确写清楚：`合同数量 != 真实库存数量`
- 本次页面新增的经营视角区块要记住：
  - 8 张报表卡片：采购 / 在途 / 库存 / 销售 / 回款 / 成本 / 利润 / 二维码
  - 演示主线与正式业务口径说明
  - 二维码与库存一致性说明
  - 最近业务推进与工单
  - 最近活动与真实库存流水
...
- 本次已完成自测：
  - `npx tsc -p apps/web/tsconfig.json --noEmit` 通过
  - `npm run build --workspace @trade-ai-demo/web` 通过
  - 已用本机无头浏览器打开 `http://127.0.0.1:5173/reports` 做页面验收
  - 已确认空白演示起点下，报表页能正确显示：
    - 真实库存与二维码为 0
    - 采购 / 回款 / 成本 / 利润使用默认演示值
    - 页面标签会明确提示 `真实统计 / 业务记录 / Demo演示`
- 本次阶段 20 完成后，主线进入：
  - `Demo 2.0：接近真实业务系统`
  - 下一阶段应从 `阶段 21：基础主数据强化` 开始
- 但当前仍然遵守你的规则：
  - 先等你验证阶段 20 效果
...
- 当前自动工单页已经支持：
  ...
  - 详情抽屉
  - 模板化 AI 工单说明
  - 关联合同 / 批次 / 单据 / 应收 / Payment 上下文展示
  - 附件要求与执行清单展示
  - 关联业务留痕展示
- 当前阶段 19 的核心实现口径已经锁定：
  - 工单中心优先复用真实 `WorkOrder` 表，不额外发明平行任务模型
  - 这一版把采购、物流、清关、财务等环节中已经生成的工单统一收口展示
  - 页面不会在打开时偷偷自动创建新工单
  - 是否生成新工单，仍然由各上游业务模块在真实节点推进时触发
  - 阶段 19 的职责是：
    - 统一展示
    - 统一提醒
    - 统一追溯
    - 用模板化 AI 说明帮助讲清每张工单的作用、风险和下一步
  - 第一版仍然不做：
    - 复杂审批流
    - 多人会签
    - 真实通知中心
...
- 这一阶段还记住一个很重要的现状：
  - 当前页面只展示数据库里真实存在的工单
  - 当前演示数据库中的工单数量会随着业务推进继续变化
  - 这说明阶段 19 现在已经把“统一工单中心”做出来了
  - 但某些后续工单是否已经生成，仍然取决于前面业务模块当前实际推进到了哪一步

## 阶段 19 后续补齐：空白演示起点下的渐进式工单生成

- 本次又补齐了一条很重要的演示原则：
  - 如果系统回到“空白演示起点”，工单也必须回到空白
  - 后续工单不能一次性全部预置
  - 必须随着业务环节逐步生成，才能更像真实 ERP 的全流程演示
- 当前最新渐进式工单生成顺序已经明确为：
  1. 上传并确认生成正式业务数据后：
     - 自动生成 `SUPPLIER_DELIVERY_FOLLOW_UP`
     - 展示为“供应商发货跟进工单”
  2. 采购推进到“国内集货完成”后：
     - 自动生成 `LOGISTICS_ARRANGEMENT`
     - 展示为“国际运输安排工单”
  3. 国际物流推进到“到达目的港”后：
     - 自动生成 `CUSTOMS_CLEARANCE`
     - 展示为“清关工单”
  4. 清关完成后：
     - 自动生成 `OVERSEAS_LAND_TRANSPORT`
     - 自动生成 `WAREHOUSE_PRE_RECEIVE`
  5. 销售单 / 出库任务创建后：
     - 自动生成 `WAREHOUSE_OUTBOUND_PICKING`
     - 展示为“仓库出库工单”
  6. 配送完成后：
     - 自动生成或刷新 `RECEIVABLE_FOLLOW_UP`
     - 展示为“财务回款跟进工单”
- 当前这一轮补齐具体落地了两件事：
  - `POST /api/documents/:id/confirm`
    - 如果正式业务数据已存在但缺少采购起始工单，系统会自动补齐
    - 如果是新生成正式业务数据，也会同步创建“供应商发货跟进工单”
  - 仓储任务上下文创建出库任务时：
    - 如果缺少 `WAREHOUSE_OUTBOUND_PICKING`，系统会自动补齐“仓库出库工单”
- 当前这条规则的边界也要记住：
  - 这次没有把所有下游业务单据提前写死到“确认生成业务数据”阶段
  - 仍然保持“业务节点到了，再生成对应工单”的方向
  - 只补齐了最前面的采购起始工单，以及销售 / 出库任务对应的仓库出库工单
  - 没有破坏现有物流、清关、预收货、财务工单的真实触发点
...
- 已尝试：
  - `npm run build --workspace @trade-ai-demo/server`
  - 结果：未完成
  - 原因：当前本地运行中的 server / Prisma Client 仍占用了 `query_engine-windows.dll.node`
  - 结论：这次阻塞仍然是 Windows 下 Prisma 引擎文件占用问题，不是阶段 19 新代码的 TypeScript 错误
- 已真实验证接口：
  - `GET /api/health`
  - `GET /api/work-orders`
  - `GET /api/work-orders/:id`
...
- 已验证当前接口真实返回：
  ...
  - 工单详情接口已能返回：
    ...
    - 执行清单与附件要求
  - 已真实验证这次补齐后的现场结果：
    - 对已生成业务数据的箱单再次调用 `POST /api/documents/:id/confirm`
    - 系统自动补齐了：
      - `WO-PROC-DEMO-202606-001`
      - 类型：`SUPPLIER_DELIVERY_FOLLOW_UP`
    - 调用 `GET /api/warehouse/workbench` 构建仓储任务上下文后
    - 系统自动补齐了：
      - `WO-OUT-BAT-DEMO-202606-001`
      - 类型：`WAREHOUSE_OUTBOUND_PICKING`

## 最近一次阶段 9 AI 流程现状感知补强

- 本次新增目标已经明确落地：
  - AI 助手不再只回答库存数字
  - 当用户问“这票货走到哪一步”“当前卡在哪个环节”“采购/物流/清关/仓储/销售/回款分别是什么状态”时
  - 必须通过外部大模型 API 基于系统当前真实业务状态回答

- 当前实现方式已经固定为：
  - 后端新增受控流程快照服务 `apps/server/src/services/aiAssistantProcess.ts`
  - 由后端统一汇总：
    - 主合同 / 主批次
    - `currentStage`
    - `nextAction`
    - `blockers`
    - `documents / procurement / logistics / customs / warehouse / sales / finance / workOrders`
    - `inventory`
    - `recentTasks / recentActivities`
  - 大模型只能读取这份后端受控 JSON
  - 不允许跳过后端受控查询直接自由猜测流程现状
  - AI 仍然不能直接写数据库
...
- 当前意图识别规则已经补强：
  ...
  - 关键词覆盖：
    ...
    - `走到哪`
    - `卡在哪`
    - `采购 / 物流 / 清关 / 仓储 / 配送 / 回款 / 工单`
  - 即使问题没有命中 `PROCESS_STATUS`
  - `OVERVIEW` 现在也会复用同一套流程快照事实源
  - 这样回答至少仍然会是“真实流程总览”，而不只是库存模板
...
- 当前大模型提示词约束也已补强：
  - 如果后端 JSON 中存在：
    ...
    - `recentActivities`
  - 模型必须把它们视为“当前真实流程现状”
  - 回答时先给结论，再简要解释，并明确下一步动作

- 本次真实自测已经完成：
  - `npx tsc -p apps/server/tsconfig.json --noEmit` 通过
  - `npx tsc -p apps/web/tsconfig.json --noEmit` 通过
  - `npm run build --workspace @trade-ai-demo/web` 通过
  - `npm run build --workspace @trade-ai-demo/server` 未完成
    - 原因仍是 Windows 下 Prisma `query_engine-windows.dll.node` 文件被运行中的进程占用
    - 这次不是新的 TypeScript 代码错误
...
- 当前这次自测时系统真实流程现状要记住：
  - 主合同：`CTR-DEMO-202606-001`
  - 主批次：`BAT-DEMO-202606-001`
  - 当前环节：`报关清关阶段`
  - 当前下一步：`完成清关，系统会自动生成境外陆运与仓库预收货任务`
  - 当前主要阻塞：`该批次还没有生成二维码，无法进入真实扫码库存链路`
  - 当前模型回答已经能把：
    - 采购状态
    - 物流状态
    - 清关状态
...
- 这一轮补强属于：
  - `阶段 9：AI 助手` 的维护增强
  - 不改变主线阶段顺序
  - 主线仍然回到后续 `阶段 20：数据报表与业务大盘`

## 最近一次阶段 9 问答口径防误解补强

- 本次新增了一条很重要的老板视角口径：
  - 当用户问“这批货现在有多少箱”“是否都出入库了”时
  - AI 不能只机械按“批次库存”返回一句“当前没有批次库存数据”
  - 否则会让老板误以为系统出错
...
- 本次代码层已经补强两件事：
  1. 老板口语化问题如：
     ...
     - `都入库了吗`
     - `都出库了吗`
     当前会优先识别为 `PROCESS_STATUS`
  2. 即使问题仍落到 `BATCH_INVENTORY / CONTRACT_INVENTORY`
     - 如果当前没有 `QrItem` 和 `StockMovement`
     - AI 也不再只说“没有批次库存数据”
     - 而是返回：
       - 合同或批次计划数量
       - 当前环节
...
- 本次真实自测结论要记住：
  - 问题：`这批货现在有多少箱 是否都出入库了`
    ...
    - 已能回答：
      ...
      - 当前主要阻塞是 `待生成二维码`
  - 问题：`这批货现在有多少箱`
    - 当前即使走 `BATCH_INVENTORY`
    - 也会明确回答：
      - 计划数量是 `100箱`
      - 但真实库存仍是 `0`
      - 原因是二维码 / 库存层尚未建立
  - 问题：`CTR-DEMO-202606-001 这个合同现在有多少箱`
    - 当前也会解释：
      - 合同总量是 `100箱`
      - 但不能直接当作当前库存

- 这次补强继续保持了原有底线：
  - 没有因为“回答更像人话”就把合同数量直接伪装成库存数量
  - AI 只是更会解释状态差异，不会篡改库存口径
...
## 最近一次阶段 23：多级二维码与仓储执行深化真实实现

- 2026-06-12 已完成 `阶段 23：多级二维码与仓储执行深化` 的第一版真实实现。
- 本次开始前已确认阶段 22 已提交：
  - Commit: `854d0c0`
  - Message: `feat: 完成阶段22合同与单据业务化深化`
- 本阶段仍然严格只处理阶段 23，没有进入阶段 24。
- 本次新增数据库与模型能力：
  - `QrItem` 增加 `unitTraceCode / boxTraceCode / palletTraceCode / freezeReason / statusRemark`。
  - `PreReceiveOrder` 增加预约到仓、波次、月台与到仓状态字段。
  - `OutboundOrder` 增加出库波次、提货清单号、双复核状态、一审/二审人员与时间、拣货状态。
  - `StocktakeOrder / StocktakeItem` 补充批次、合同、计划数量、实盘数量、差异数量、库位记录等字段。
  - 新增 `WarehouseAnomaly`，用于记录入库、出库、库存环节的错货、漏扫、多扫、破损、少件、多件等异常。
- 本次新增后端能力：
  - 生成二维码时同步生成最小包装码、箱码、托盘码。
  - `GET /api/qr-items` 与批次详情接口返回多级码、冻结原因和状态备注。
  - 仓储工作台返回预收货预约号、波次号、月台号、出库波次、提货清单、双复核状态、异常记录、盘点任务和库位容量占用。
  - `POST /api/warehouse/anomalies` 上报仓储异常。
  - `POST /api/warehouse/anomalies/:id/handle` 处理仓储异常。
  - `POST /api/warehouse/qr-items/:id/freeze` 冻结在库二维码，并写入 `StockMovement(FREEZE)`。
  - `POST /api/warehouse/qr-items/:id/unfreeze` 解冻二维码，并写入 `StockMovement(UNFREEZE)`。
  - `POST /api/warehouse/stocktakes` 创建盘点任务。
  - `POST /api/warehouse/stocktakes/:id/complete` 完成盘点并记录差异。
  - `POST /api/warehouse/outbound-orders/:id/first-review` 完成出库一审。
  - `POST /api/warehouse/outbound-orders/:id/second-review` 完成出库二审。
  - 出库二审不能跳过一审。
  - 扫码出库接口已增加硬校验：出库单未完成双人 / 双岗复核时，不允许扫码出库。
  - 批量扫码如果全部失败，会返回失败状态，不再出现 `processed = 0` 但任务状态被推进的问题。
- 本次前端已实现：
  - `仓储管理` 页面增加多级二维码、仓储异常、盘点任务、冻结 / 解冻、出库双复核等执行区。
  - `二维码追溯` 页面展示最小包装码、箱码、托盘码、冻结原因和状态备注。
  - `批次追踪` 页面展示批次多级码汇总、二维码明细和库存流水。
  - `库存统计` 区域展示多级码统计、库位容量占用、冻结原因、盘点任务等仓储执行指标。
- 本次真实接口自测结果：
  - 从空白演示起点开始，上传标准合同图片与箱单图片。
  - AI Mock 识别成功，并通过“合同 + 箱单”门槛生成正式 `Contract / Batch / PurchaseOrder / Payment / Receivable`。
  - 生成 100 个二维码，同时生成 100 个最小包装码、100 个箱码、10 个托盘码。
  - 预收货上下文返回预约号、入库波次和月台号。
  - 批量入库 5 箱成功，并写入入库 `StockMovement`。
  - 未完成双复核时尝试出库，被后端拒绝。
  - 冻结 1 个在库二维码成功，冻结库存统计增加。
  - 冻结二维码尝试出库，被后端拒绝。
  - 解冻二维码成功，并写入解冻流水。
  - 创建并完成盘点任务成功。
  - 创建并处理仓储异常成功。
  - 完成出库一审和二审后，批量出库 2 箱成功。
  - 最终库存统计验证：总码 100、真实在库 3、已出库 2，库存仍严格由 `QrItem.status` 与 `StockMovement` 计算。
- 本次构建与页面检查：
  - `npm run prisma:migrate --workspace @trade-ai-demo/server` 通过，阶段 23 迁移已应用。
  - `npx tsc -p apps/server/tsconfig.json --noEmit` 通过。
  - `npx tsc -p apps/web/tsconfig.json --noEmit` 通过。
  - `npm run build --workspace @trade-ai-demo/web` 通过，仅保留 Vite 大 chunk 提示。
  - `npm run build --workspace @trade-ai-demo/server` 通过。
  - 浏览器检查 `http://127.0.0.1:5173/warehouse`：仓储管理页面可正常挂载，无 Vite 错误覆盖层。
  - 浏览器检查 `http://127.0.0.1:5173/qr-items`：二维码追溯页面可正常挂载，能看到箱码 / 托盘码相关展示。
- 测试完成后已调用 `POST /api/setup/reset-demo`，系统恢复为空白演示起点：
  - `documents = 0`
  - `contracts = 0`
  - `batches = 0`
  - `qrItems = 0`
  - `stockMovements = 0`
  - `warehouseAnomalies = 0`
  - `stocktakeOrders = 0`
  - `workOrders = 0`
- 当前收口状态：
  - 阶段 23 已实现并自测通过。
  - 阶段 23 的代码和文档变更尚未提交 Git。
  - 按用户规则，下一步先等待用户打开页面验证阶段 23 效果。
  - 用户确认满意后，再执行阶段 23 的 `git add / git commit / git push`。
  - 阶段 23 提交完成后，才能进入 `阶段 24：采购、物流、清关真实任务化`。

## 最近一次信息架构修正：批次追踪不占左侧一级主菜单

- 2026-06-12 已按成熟 ERP 设计原则重新固化批次定位。
- 本次原则优先级高于历史记录中所有“批次数据作为独立主菜单”的说法。
- 新原则：
  - `批次` 不是独立业务处理模块。
  - `批次` 是贯穿合同、采购、物流、清关、仓储、销售、财务、报表和 AI 问答的跨模块追踪对象。
  - 左侧一级菜单只放真正的一线业务模块或经营管理模块。
  - `/batches` 路由继续保留，但页面名称和定位改为 `批次追踪`。
  - `批次追踪` 是二级查询 / 追踪入口，不占用左侧一级菜单。
- 本次页面归位规则：
  - 正式批次的生成：仍在 `合同与单据` 中由人工确认单据后生成。
  - 采购、集货、物流、清关、仓储、销售、财务等对批次的处理：在各自业务模块内完成。
  - 二维码生成：归位到 `二维码追溯` 页面中按批次生成。
  - 入库、冻结、盘点、出库：归位到 `仓储管理` 页面。
  - `批次追踪` 页面只展示批次列表、批次详情、多级码汇总、库存流水和跳转入口。
- 冗余一级栏目归位规则：
  - `合同数据` 作为 `合同与单据` 下的正式业务数据查看入口，不占左侧一级菜单。
  - `批次追踪` 作为跨模块追踪对象，不占左侧一级菜单。
  - `扫码入库 / 扫码出库 / 库存统计` 归入 `仓储管理`。
  - `回款核销 / Payment` 归入 `财务回款`。
  - `AI 助手` 归入全局右下角悬浮入口，不占左侧一级菜单。
  - 未来 `手机扫码端` 可以独立部署为移动 H5，但业务归属仍是 `仓储管理`，不能绕过后端接口直接写库。
- 本次代码侧已同步：
  - 左侧菜单移除独立 `批次数据`。
  - `/batches` 页面标题改为 `批次追踪`。
  - `/batches` 进入时左侧仍兼容选中 `合同与单据`，表示批次来自单据确认后的业务资料层。
  - `/batches` 页面不再直接执行生成二维码动作，而是跳转到 `二维码追溯`。
  - `合同与单据` 中的入口文案改为 `查看批次追踪`。
  - 顶部文案同步为 `14 项侧栏模块 + 全局 AI 助手入口已启用`。
- 当前阶段状态仍然不变：
  - 这次调整属于阶段 23 收尾的信息架构修正。
  - 阶段 23 仍未提交 Git。
  - 下一步仍然等待用户验证阶段 23 和本次批次菜单调整。

## 最新状态：客户交付包已生成并同步 GitHub

- 2026-06-22 已将客户交付包整理完成，压缩包复制到 `D:\trade-ai-demo-ascii\trade-ai-demo-customer-package.zip`。
- 压缩包内已包含前端界面、DeepSeek 调用、`需求` 资料、`pics` 测试单据，以及 `README.txt` / `使用说明.txt`，客户可直接打开测试。
- 本次客户版相关代码、文档和打包脚本已提交并推送到 GitHub `origin/main`。
- 后续如果继续收口，可以考虑再做安装器、单文件启动器或更简化的客户版封面。
